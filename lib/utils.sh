...
# Link dotfiles helper scripts into ~/.local/bin so they are available on PATH
setup_dotfiles_bin() {
    local dotfiles_dir="${1:-$(get_dotfiles_dir)}"
    local source_dir="$dotfiles_dir/bin"
    local target_dir="$HOME/.local/bin"
    local script
    local count=0

    if [ ! -d "$source_dir" ]; then
        log_debug "No dotfiles bin directory found: $source_dir"
        return 0
    fi
    ...
    for script in "$source_dir"/*; do
        [ -f "$script" ] || continue
        chmod +x "$script" 2>/dev/null || log_warning "Could not make executable: $script"
        safe_symlink "$script" "$target_dir/$(basename "$script")"
        count=$((count + 1))
    done

    if [ "$count" -gt 0 ]; then
        log_success "Linked $count dotfiles script(s) into $target_dir"
    else
        log_debug "No dotfiles scripts found in $source_dir"
    fi
}
